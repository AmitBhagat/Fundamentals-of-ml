...
def ultimate_rehab_v2(backup_path, target_path):
    with open(backup_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Title Extraction (Handle the </h1>)
    title_match = re.search(r'Chapter \d+: (.*?)(?: </h1>)?\n', content)
    title = title_match.group(1).strip() if title_match else Path(backup_path).stem.split('_', 1)[-1].replace('_', ' ').title()
    # Clean any residual HTML from title
    title = re.sub(r'<.*?>', '', title).strip()

    # 2. Cleanup body start
    content = re.sub(r'<h1.*?>.*?</h1>', '', content, flags=re.DOTALL)

    # 3. Convert HTML boxes to Modern Alerts
    def repl_box(match):
...
for p in backup_dir.glob('*.md'):
    try:
        ...
        if file_id in file_to_subject:
            ...
            ultimate_rehab_v2(p, content_dir / subject / target_name)
    except: continue

print("THE ULTIMATE REHAB V2 COMPLETE.")
